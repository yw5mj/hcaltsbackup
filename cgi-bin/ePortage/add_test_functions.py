...
def add_test_template(card_id, serial_number, suggested_test):
    if not suggested_test:
        suggested_test=0
    db = connect(0)
    cur = db.cursor()

    card_unique_id = module_functions.Portage_fetch_uniqueID(serial_number)
    ...
    print					'<h1>Add test for Card %d<br>(%s)</h1>' %(serial_number,card_unique_id)
    print				'</div>'
    print			'</div>'
